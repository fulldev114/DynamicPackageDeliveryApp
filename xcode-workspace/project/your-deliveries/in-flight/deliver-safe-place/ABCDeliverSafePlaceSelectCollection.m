//
//  ABCDeliverSafePlaceSelectCollection.m
//  your-dpd
//
...
@interface ABCDeliverSafePlaceSelectCollection()
	@property (nonatomic) NSUInteger radioSelectIndex;
	@property (nonatomic) NSArray* selectCollection;
@end

@implementation ABCDeliverSafePlaceSelectCollection
	...
	- (instancetype) initWithSelectCollection:(NSArray*)selectCollection {
		...
		[self setSelectCollection:selectCollection];
		...
		[self setSizeView:CGSizeMake(
			...
			[self SizeHeadingHeight] + ([self SizeRowHeight] * [[self selectCollection] count]))];
	...
	- (UITableViewCell*)
		...
		ABCDeliverSafePlaceSelect* view = [[ABCDeliverSafePlaceSelect alloc]
			initWithTitle:[[self selectCollection] objectAtIndex:[indexPath row]]
	...
	- (NSInteger)
		...
		return [[self selectCollection] count];
	}
	...
	-(UIView*)
		...
		[label setText:NSLocalizedString(@"Select your safe place", nil)];
